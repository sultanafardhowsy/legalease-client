...
const UpdateUserModal = () => {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      form.reset();
      setIsOpen(false); // ✅ replaces document.getElementById(...).close()
    } catch (error) {
  ...
  return (
    <>
      ...
      <button
        ...
        onClick={() => setIsOpen(true)} // ✅ replaces document.getElementById(...).showModal()
      >
        <PencilToLine className="w-5 h-5" />
        Update Profile
      </button>

      {/* Modal — only mounted when open, so no SSR/client mismatch */}
      {isOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="max-w-md w-full rounded-2xl bg-white dark:bg-slate-900 border border-slate-100 dark:border-blue-500/20 text-slate-800 dark:text-white shadow-2xl p-6 sm:p-8 relative">

            {/* Close Button */}
            <button
              className="btn btn-sm btn-circle btn-ghost absolute right-4 top-4 text-slate-400 hover:text-slate-600 dark:text-slate-400 dark:hover:text-white"
              onClick={() => setIsOpen(false)} // ✅ replaces document.getElementById(...).close()
            >
              ✕
            </button>
